...
As it is simply a matter of extracting some HTML content,p `beautifulsoup` should be sufficient!
...
We will, however, disregard the 'standard stats' and 'miscellaneous stats', Fortunately the URLs are similar and only differ by a simple substring.
...
However, the one and only thing I really miss about R is how straightforward and concise the syntax is for the most popular data wrangling libraries.
...
What even is this?!
Four positions: we would expect four 'clusters', right?
...
I would rather not go into too much detail of this, but essentially: the Euclidean metric in this case does not make sense.
...
![Much better](https://media1.tenor.com/m/o1fnLBZm-OAAAAAC/phew-sigh.gif)
...
A default value of 0.1 is fine for our use case, which leads us to the following plots for 2D and 3D UMAP embeddings.
