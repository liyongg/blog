...
gross_salaries = 1000 * np.linspace(start=salary_start, stop=salary_stop, num=steps)
...
Theoretically, you cannot pay more than 49,5% of your salary to taxes as it is hardcapped by the tax brackets - $y=0.495$ is an asymptote in 2024.
...
```{python}
x = df['net_salary_yearly'].to_numpy()
...
fig = px.scatter(df, x="net_salary_yearly", y="gross_salary_yearly")
fig.add_trace(go.Scatter(x=x, y=y_pred.reshape(x.shape), mode='markers'))
...
```{python}
# y_pred = model.predict(x.reshape(-1, 1))
y_pred = regressor.predict(gross_salaries.reshape(-1, 1))


fig = px.scatter(x=y, y=y_pred)
fig.add_trace(go.Scatter(x=y, y=x, mode='markers'))
